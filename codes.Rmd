...
cols_to_mutate <- c("White_wine.0.0", "White_wine.1.0", "White_wine.2.0", "White_wine.3.0", 
                    "White_wine.4.0", "Beer.0.0", "Beer.1.0", "Beer.2.0", "Beer.3.0", 
                    "Beer.4.0", "Spirits.0.0", "Spirits.1.0", "Spirits.2.0", "Spirits.3.0", 
                    "Spirits.4.0")

data <- data %>% 
  mutate_at(vars(cols_to_mutate), 
            list(~ recode(., `555` = 0.5, `600` = 6, `111` = NA_real_)))
...
data <- data %>%
  ...
  mutate(White_mean = mean(c_across(starts_with("White_")), na.rm = TRUE),
         Rose_mean = mean(c_across(starts_with("Rose_")), na.rm = TRUE),
         Red_mean = mean(c_across(starts_with("Red_")), na.rm = TRUE)) %>%
  ungroup()

# Create Beer_mean, Spirits_mean
data <- data %>%
  rowwise() %>%
  mutate(Beer_mean = mean(c(Beer.0.0, Beer.1.0, Beer.2.0, Beer.3.0, Beer.4.0), na.rm = TRUE),
         Spirits_mean = mean(c(Spirits.0.0, Spirits.1.0, Spirits.2.0, Spirits.3.0, Spirits.4.0), na.rm = TRUE)) %>%
  ungroup()
