...
```{r}
# create new White_i columns
for (i in 0:4) {
  data <- transform(data, 
                    White_i = ifelse(!is.na(Size_White_i) & !is.na(White_wine.i.0), Size_White_i * White_wine.i.0, 
                                    ifelse(is.na(Size_White_i) & !is.na(White_wine.i.0), 1.4 * White_wine.i.0, 
                                           NA)))
  names(data)[names(data) == "White_i"] <- paste("White_", i, sep="")
}

# create new White_mean column
data$White_mean <- rowMeans(data[ , grep("White_", names(data))], na.rm = TRUE)
